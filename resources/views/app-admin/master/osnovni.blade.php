    ...
    <body>
            ...
            <div class="container">
                ...
                <div id="dMenija" class="collapse navbar-collapse">
                    <ul class="nav navbar-nav navbar-right">
                        ...
                        <li><a href="/administracija/magacin" data-toggle="tooltip" title="Magacin" data-placement="bottom"><i class="glyphicon glyphicon-folder-open"></i></a></li>
                        ...
                        <li><a href="/administracija/osnovna-podesavanja" data-toggle="tooltip" title="Osnovna podešavanja" data-placement="bottom"><i class="glyphicon glyphicon-cog"></i></a></li>
                        <li><a href="/administracija/verzioniranje" data-toggle="tooltip" title="Razvoj platforme" data-placement="bottom"><i class="glyphicon glyphicon-list"></i></a></li>
        ...
        <div class="container" style="width: 98%">
            <div class="col-sm-4 form-inline">
                    ...
                    <a href="/administracija/uputstvo" class="btn btn-sm btn-success" style="margin-left: 3px;padding:3px 10px"><i class="glyphicon glyphicon-tag"></i> Uputstvo</a>
                    ...
                    <div id="korpa">
                        <h3>
                            ...
                            <button id="korpaINIC" style="display: none" class="btn btn-xs btn-info" data-toggle="tooltip" title="Evidentiraj početno stanje" onclick="fakturisanje.kreiraj(5)"><i class="glyphicon glyphicon-log-in"></i></button>
        ...
        $(function(){
            korpa.ucitaj();
            @if(Session::get('jezik')!='sr-i')
                @if(Session::get('jezik')=='sr-e')
                    fakturisanje.setTekst('jedinicaMjere','Jedinica {{trans('osnovno.mjere')}}');
                    fakturisanje.setTekst('maloprodajnaCijena','Maloprodajna {{trans('osnovno.cijena')}}');
                    fakturisanje.setTekst('provjeritePodatkeIzvrsi','{{trans('osnovno.provjerite')}} podatke i pristupite izvršenju.');
                    fakturisanje.setTekst('stolaza','{{trans('osnovno.stolaza')}}');
                    fakturisanje.setTekst('akcijaJeUsjesna','Akcija je {{trans('osnovno.uspjesno')}} izvršena.');
                    fakturisanje.setTekst('proizvodiNisuUMag','Sledeći proizvodi se ne nalaze u ciljanom magacinu. Unesite potrebne podatke i sačuvajte {{trans('osnovno.promjene')}}.');
                    fakturisanje.setTekst('cijena','{{trans('osnovno.cijena')}}'.charAt(0).toUpperCase() + '{{trans('osnovno.cijena')}}'.slice(1));
                @endif
            @endif
            fakturisanje.setToken('{{csrf_token()}}')
        });
        var korpa={
            token:'{{csrf_token()}}',
        ...
        var tp={
            ...
            posalji:function(){
                $.post('/posalji',{_token:tp.token,naslov:$('[name=naslov]').val(),poruka:$('[name=poruka]').val()},function(data){
                    $('.modal-body').html(data);
                })
            }
        }
        function pretraga(val,istekZaliha,idMagacin){
            ...
            $.post('/administracija/proizvod/pretraga',{
                            ...
                            '<thead>' +
                                '<tr>' +
                                    ...
                                    ($('#samoMagacin').is(':checked')?
                                        ...
                                        '<th><i class="glyphicon glyphicon-resize-horizontal" data-toggle="tooltip" title="{{\App\OM::ttrans('osnovno.stolaza')}}"></i></th>' +
                ...
                for(var i=0; i<rezultat.length; i++) {
                    ...
                    ($('#samoMagacin').is(':checked')?'<td><a href="#" onclick="pretraga(null,null,'+rezultat[i]['id']+')">' + rezultat[i]['nazivmagacina'] + '</a></td>':'') +
